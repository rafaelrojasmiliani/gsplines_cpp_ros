...
main(){
    ...
    mygroup=$(id -g -n $USER)
    myuser="$USER"
    docker pull rafa606/vim-ros-noetic-gsplines
    docker run -it --rm \
        ${DOCKER_VIDEO_OPTIONS} \
        --volume $(pwd)/../:/workspace/src: \
        --entrypoint="/bin/bash" \
        --privileged \
        "rafa606/vim-ros-noetic-gsplines" -c "addgroup --gid ${mygid} ${mygroup} --force-badname;  adduser --gecos \"\" --disabled-password  --uid ${myuid} --gid ${mygid} ${myuser} --force-badname ; usermod -a -G video ${myuser}; echo ${myuser} ALL=\(ALL\) NOPASSWD:ALL >> /etc/sudoers; sudo -EHu ${myuser}  bash"
}
